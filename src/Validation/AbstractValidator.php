...
abstract class AbstractValidator
{
    ...
    public function __construct(Container $container)
    {
        ...
        $this->serviceProvider = $this->request['service_provider'];
    ...
    public function validate($removeRequired = false)
    {
        ...
        $this->validator = $this->container['validator']->make($content, $this->getRules(), $messages);
        /** get iterable keys and iterate */
        $mappings = $this->mappings();
        foreach ($mappings as $key => $value) {
            if (class_exists($value) && array_get($content, $key)) {
                $class = new $value($this->container);
                $this->validator->iterate($this->request, $key, $class, $class->messages());
            }
        }
       ...
       foreach ($mappings as $inputKey => $dbValue) {
            if (is_array($input) && array_get($input, $inputKey) !== null) {
                ...
                if ($value instanceof UploadedFile) {
                    ...
                    if (class_exists($mappings[$inputKey])) {
                        $class = new $mappings[$inputKey]($this->container);
                        foreach ($value as $v) {
                            $response[$inputKey][] = $this->getDbValues($v, $class->mappings());
                        }
                    } else {
                        array_set($response, $dbValue, $value);
                    }
